...
> Consolidated layout — CLI and domain remain separated; the rest are merged where the source structure was overly fragmented.
...
│
├── bta/
│   ├── cli/ — keep unchanged; user-facing command entry points and thin command wrappers around the orchestrator
...
│   │
│   ├── orchestrator/ — workflow control, state transitions, retries, and trace capture
...
│   │
│   ├── github/
│   │   └── service.py — unified GitHub layer for auth, issue fetch, repo clone, branching, and PR publishing
│   │
│   ├── ai/
